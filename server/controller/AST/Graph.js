...
const TIPO_OPERACION = require("../Enum/TipoOperaciones");
...
node [shape=circle style=filled color=\"greenyellow\" fontname= \"Ubuntu\" fontsize=\"14\"];
edge[arrowhead = icurve color=\"chocolate4\" penwidth=\"1.5\"];`
    ...
    graph_Instruccion(_up, _instruccion) {
        if (_instruccion.tipo === TIPO_INSTRUCCION.PRINT) {
            ...
            this.count++;
            this.grafo += child + '[label = \"WRITELINE\"];\n';
        ...
        else if (_instruccion.tipo === TIPO_INSTRUCCION.START) {
            var child = 'Node' + this.count;
            this.count++;
